...
char set::nextElement(char* (&ptr)) {
	while (isspace(*ptr)) ptr++; // Eating space elements before symbol

	char symbol = *ptr;
	ptr++;

	while (isspace(*ptr)) ptr++; // Eateing space elents after symbol

	if (*ptr != ',' && *ptr) {
		printf("Error! Wrong input format!!! Expected \",\" but found:\n%s\n", ptr);
		system("pause");
		exit(-1);
	}
